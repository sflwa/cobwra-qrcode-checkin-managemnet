 ...
 * Update: Added Announced Guest UI Section & Removed CSV Export.
...
class COBWRA_Dashboard {
	...
	public function __construct( $engine ) {
		$this->engine = $engine;
		// Use priority 1 to catch actions before HTML output.
	...
	public function handle_actions() {
		...
		// Handle Sync only - CSV Export removed per request.
	...
	public function render_dashboard() {
		...
		// 2. Fetch All Scans from the Temporary Scanner Form.
		...
		// Initialize buckets.
		$groups            = array( 'CONFLICT' => array(), 'VACANCY' => array(), 'ROLE MISMATCH' => array() );
		$checked_in_reps  = array();
		$announced_guests = array();
		$public_count     = 0;
		$announced_count  = 0;

		// 3. Process Scans via Engine.
		foreach ( $scans as $s ) {
			$res = $this->engine->analyze_scan( $s->rsvp_id );

			// Handle Reps (Flag 0 = Valid check-in).
			if ( 0 === $res['flag'] ) {
			...
			if ( 'ANNOUNCED' === $res['status'] ) {
				$announced_count++;
				$announced_guests[] = $res; // Store full object for UI section.
				continue;
			}

			// Add to discrepancy log groups.
		...
		// 4. Identify Missing Communities.
		...
		<style>
			...
			.action-table-container { background:#fff; padding:20px; border:1px solid #ddd; border-top:none; border-radius: 0 0 8px 8px; }
		</style>
		...
		<div class="cobwra-dash">
			...
			<div class="grid-box">
				...
				if ( ! empty( $announced_guests ) ) {
					...
					foreach ( $announced_guests as $ag ) { 
						// Uses organization if available, otherwise just name.
						$display = ! empty( $ag['comm'] ) ? $ag['name'] . ' (' . $ag['comm'] . ')' : $ag['name'];
						echo "<div>{$i}. " . esc_html( $display ) . '</div>'; 
						$i++; 
					} 
				} else {
			...
			<div class="action-table-container">
				...
				<table class="wp-list-table widefat fixed striped">
					...
					<tbody>
						<?php 
						foreach ( array( 'CONFLICT', 'VACANCY', 'ROLE MISMATCH' ) as $k ) {
